...
class Repeat(GateWithRegisters):
    ...
    def __init__(
            self,
            subbloq: GateWithRegisters | cirq.Gate,
            *args,
            n_repetitions: int = 1,
            caching: bool = False,
            cirq_quregs = None,
        ...
        '''
            ...
            :: quregs :: Map back to cirq qubit labels for qualtran bloq
        '''
        ...
        if issubclass(type(subbloq), qualtran.Bloq):
            ...
            raise TypeError(f"{subbloq} is not a qualtran.Bloq, cirq.Gate or a cirq.Circuit")
    ...
    def _qualtran_bloq_decomp(self):
        '''
            _qualtran_decomp
            Qualtran decomposer
        '''
        yield self.subbloq.to_cirq_circuit(*self.args, cirq_quregs=self.cirq_quregs, **self.kwargs)
    ...
    def decompose_from_registers(
        ...
        **quregs: NDArray[cirq.Qid]
        ...
        '''
        if self.caching: 
            cached_obj = list(self.subbloq.decompose_from_registers(*args, context, quregs)) 
            ops = lambda : iter(cached_obj) 
        else:
    ...
    def _decompose_with_context_(self, *args, context=None, **kwargs):
        if context is None:
            context = cirq.DecompositionContext(
                cirq.ops.SimpleQubitManager()
            )

        for obj in self.decompose():
            yield obj 
    ...
    def decompose(self):
        ...
        '''
        for _ in range(self.n_repetitions): 
            for obj in self.decompose_once():
                yield obj
